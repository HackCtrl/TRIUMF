...
## Суpabase + Vercel (для продакшна)

Проект содержит serverless API-функции (`api/applications/*`) для Vercel, которые автоматически работают с Supabase.

1. Подробная инструкция: см. `SUPABASE_SETUP.md` в корне проекта.
2. Коротко:
   - Создайте проект в Supabase (бесплатно на https://supabase.com/)
   - Выполните SQL из `SUPABASE_SETUP.md` (создаст таблицу)
   - Добавьте переменные окружения в Vercel:
     - `SUPABASE_URL`
     - `SUPABASE_SERVICE_ROLE_KEY`
     - `ADMIN_PASS` (пароль для админ-панели, по умолчанию `301062Ki`)
   - Деплойте на Vercel (git push)
3. Результат: заявки будут сохраняться в Supabase и видны всем администраторам через админ-панель.
